    ...
    if ($user['test_account'] == 'no') {
        ...
        try {
            ...
            if ($panel_fetch['type'] == 'sanayi') {
                ...
                # ---------------- check errors ---------------- #
                if ($create_status['status'] == false) {
                    sendMessage($from_id, sprintf($texts['create_error'], 1), $me_manage);
                    exit();
                }
                # ---------------------------------------------- #
                $sql->query("UPDATE `users` SET `count_service` = count_service + 1, `test_account` = 'yes' WHERE `from_id` = '$from_id'");
                ...
                deleteMessage($from_id, $message_id + 1);
                sendMessage($from_id, sprintf($texts['create_test_account'], $test_account_setting['time'], $link, $panel_fetch['name'], $test_account_setting['time'], $test_account_setting['volume'], base64_encode($code)), $me_manage);
            }
        } catch (\Throwable $e) {
        ...
        sendMessage($from_id, $texts['already_test_account'], $me_manage);
    }
}
...
elseif($text == 'حساب کاربری من 💼'){
    sendMessage($from_id, "دوست عزیز به حساب کاربری خودتون خوش آمدید 😉", $me_manage);
}
elseif($text == 'دریافت لینک💎'){
	 bot('sendMessage',[
	'chat_id'=>$from_id,
	'caption'=>"
📣 $channelname
...
👆👆 همین حالا استارتش کن 👆👆
",
    		])->result->message_id;
    bot('sendmessage',[
	'chat_id'=>$from_id,
	'text'=>"شما با ارسال لینک پایین میتوانید دوستان خود را دعوت کنید سپس مقداری سکه بصورت خودکار واریز به حساب شما میشود 🎁
	t.me/$usernamebot?start=$from_id
...
elseif ($text == '👤 پروفایل') {
    ...
    sendMessage($from_id, sprintf($texts['my_account'], $from_id, number_format($user['coin']), $count_all, $count_all_active, $count_all_inactive), $me_manage);
}
...
# ------------ free test code admins ------------ #
$test_account_setting['status'] == 'active';
$admins = $sql->query("SELECT * FROM `admins`")->fetch_assoc() ?? [];
if ($from_id == $config['dev'] or in_array($from_id, $admins)) {
if (in_array($text, ['🎁 سرویس تستی (مخصوص مدیران)'])) {
    step('panel');
    if ($user['test_account'] == 'yes') {
        sendMessage($from_id, '⏳', $panel);
        ...
        try {
		    ...
		    $subscribe = (strpos($create_status['subscription_url'], 'http') !== false) ? $create_status['subscription_url'] : $panel_fetch['login_link'] . $create_status['subscription_url'];
                    ...
                    $sql->query("INSERT INTO `test_account` (`from_id`, `location`, `date`, `volume`, `link`, `price`, `code`, `status`) VALUES ('$from_id', '{$panel_fetch['name']}', '{$test_account_setting['date']}', '{$test_account_setting['volume']}', '$links', '0', '$code', 'active')");
                    deleteMessage($from_id, $message_id + 1);
                    sendMessage($from_id, sprintf($texts['create_test_account_admin'], $test_account_setting['time'], $subscribe, $panel_fetch['name'], $test_account_setting['time'], $test_account_setting['volume'], base64_encode($code)), $start_key);
                } else {
                    deleteMessage($from_id, $message_id + 1);
                    sendMessage($from_id, sprintf($texts['create_error'], 1), $start_key);
                }
            ...
            if ($panel_fetch['type'] == 'sanayi') {
                ...
                $sql->query("INSERT INTO `test_account` (`from_id`, `location`, `date`, `volume`, `link`, `price`, `code`, `status`) VALUES ('$from_id', '{$panel_fetch['name']}', '{$test_account_setting['date']}', '{$test_account_setting['volume']}', '$link', '0', '$code', 'active')");
                deleteMessage($from_id, $message_id + 1);
                sendMessage($from_id, sprintf($texts['create_test_account_admin'], $test_account_setting['time'], $link, $panel_fetch['name'], $test_account_setting['time'], $test_account_setting['volume'], base64_encode($code)), $panel);
            }
        } catch (\Throwable $e) {
            sendMessage($config['dev'], $e);
        }
        ...
        sendMessage($from_id, $texts['already_test_account'], $panel);
    }
}
# ------------ panel ------------ #
$admins = $sql->query("SELECT * FROM `admins`")->fetch_assoc() ?? [];
if ($from_id == $config['dev'] or in_array($from_id, $admins)) {
    ...
    elseif ($data == 'alireza') {
        alert('❌ در حال تکمیل کردن این بخش هستیم لطفا صبور باشید !', true);
       exit();
       //step('add_server_alireza');
       // deleteMessage($from_id, $message_id);
       //sendMessage($from_id, "‌👈🏻⁩ اسم پنل خود را به دلخواه ارسال کنید :↓\n\nمثال نام : 🇳🇱 - هلند\n• این اسم برای کاربران قابل نمایش است.", $cancel_add_server);
    }
    
    elseif ($user['step'] == 'add_server_alireza') {
        if ($sql->query("SELECT `name` FROM `panels` WHERE `name` = '$text'")->num_rows == 0) {
            step('send_address_alireza');
            file_put_contents('add_panel.txt', "$text\n", FILE_APPEND);
            sendMessage($from_id, "🌐 آدرس لاگین به پنل را ارسال کنید.\n\n- example:\n http://1.1.1.1:8000\n http://1.1.1.1:8000/vrshop\n http://domain.com:8000", $cancel_add_server);
        } else {
            sendMessage($from_id, "❌ پنلی با نام [ <b>$text</b> ] قبلا در ربات ثبت شده !", $cancel_add_server);
    ...
    elseif ($user['step'] == 'send_address_alireza') {
        if (preg_match("/^(http|https):\/\/(\d+\.\d+\.\d+\.\d+|.*)\:.*$/", $text)) {
            if ($sql->query("SELECT `login_link` FROM `panels` WHERE `login_link` = '$text'")->num_rows == 0) {
                step('send_username_alireza');
                file_put_contents('add_panel.txt', "$text\n", FILE_APPEND);
                sendMessage($from_id, "🔎 - یوزرنیم ( <b>username</b> ) پنل خود را ارسال کنید :", $cancel_add_server);
            } else {
            sendMessage($from_id, "❌ پنلی با ادرس [ <b>$text</b> ] قبلا در ربات ثبت شده !", $cancel_add_server);
    ...
    elseif ($user['step'] == 'send_username_alireza') {
        step('send_password_alireza');
        file_put_contents('add_panel.txt', "$text\n", FILE_APPEND);
        sendMessage($from_id, "🔎 - پسورد ( <b>password</b> ) سرور خود را ارسال کنید :", $cancel_add_server);
    }
    
    elseif ($user['step'] == 'send_password_alireza') {
        step('none');
        $info = explode("\n", file_get_contents('add_panel.txt'));
        $response = loginPanelAlireza($info[1], $info[2], $text);
        if ($response['success']) {
            $code = rand(11111111, 99999999);
            $session = str_replace([" ", "\n", "\t"], ['', '', ''], explode('session	', file_get_contents('cookie.txt'))[1]);
            ...
            $sql->query("INSERT INTO `alireza_panel_setting` (`code`, `inbound_id`, `example_link`, `flow`) VALUES ('$code', 'none', 'none', 'offflow')");
            sendMessage($from_id, "✅ ربات با موفقیت به پنل شما لاگین شد!\n\n▫️یوزرنیم : <code>{$info[2]}</code>\n▫️پسورد : <code>{$text}</code>\n▫️کد پیگیری : <code>$code</code>", $manage_server);
        } else {
            sendMessage($from_id, "❌ لاگین به پنل با خطا مواجه شد , بعد از گذشت چند دقیقه مجددا تلاش کنید !\n\n🎯 دلایل ممکن متصل نشدن ربات به پنل شما :↓\n\n◽باز نبودن پورت مورد نظر\n◽باز نشدن آدرس ارسالی\n◽آدرس ارسالی اشتباه\n◽یوزرنیم یا پسورد اشتباه\n◽قرار گرفتن آی‌پی در بلاک لیست\n◽️باز نبودن دسترسی CURL\n◽️مشکل کلی هاست", $manage_server);
        }
    ...
    elseif (strpos($data, 'change_status_panel-') !== false) {
        $code = explode('-', $data)[1];
        $info_panel = $sql->query("SELECT * FROM `panels` WHERE `code` = '$code'")->fetch_assoc();
        if ($info_panel['type'] == 'alireza') {
            $alireza_setting = $sql->query("SELECT * FROM `alireza_panel_setting` WHERE `code` = '{$info_panel['code']}'")->fetch_assoc();
            if ($alireza_setting['example_link'] == 'none') {
                alert('🔴 برای روشن کردن پنل علی رضا ابتدا باید اینباند آیدی و نمونه سرویس را تنظیم کنید !');
                exit;
            } elseif ($alireza_setting['inbound_id'] == 'none') {
                alert('🔴 برای روشن کردن پنل علی رضا ابتدا باید اینباند آیدی و نمونه سرویس را تنظیم کنید !');
                exit;
            }
        }
    ...
    elseif (strpos($data, 'status_panel-') !== false or strpos($data, 'update_panel-') !== false) {
        ...
        $info = [
            'ip' => explode(':', str_replace(['http://', 'https://'], '', $info_server['login_link']))[0] ?? '⚠️',
            'port' => explode(':', str_replace(['http://', 'https://'], '', $info_server['login_link']))[1] ?? '⚠️',
            'type' => ($info_server['type'] == 'alireza') ? 'مرزبان' : 'علی رضا',
        ];
    ...
    elseif (strpos($data, 'set_inbound_alireza') !== false) {
        $code = explode('-', $data)[1];
        step('send_inbound_id-'.$code);
        sendMessage($from_id, "👇 آیدی سرویس مادر که قرار است کلاینت ها داخل آن اد شود را ارسال کنید : ( id ) :", $back_panel);
    }
    ...
    elseif (strpos($user['step'], 'send_inbound_id') !== false) {
        if (is_numeric($text)) {
            ...
            $info_panel = $sql->query("SELECT * FROM `panels` WHERE `code` = '$code'")->fetch_assoc();
            include_once 'api/alireza.php';
            $xui = new Alireza($info_panel['login_link'], $info_panel['token']);
            $id_status = json_decode($xui->checkId($text), true)['status'];
            if ($id_status == true) {
                step('none');
                if ($sql->query("SELECT * FROM `alireza_panel_setting` WHERE `code` = '$code'")->num_rows > 0) {
                    $sql->query("UPDATE `alireza_panel_setting` SET `inbound_id` = '$text' WHERE `code` = '$code'");
                } else {
                    $sql->query("INSERT INTO `alireza_panel_setting` (`code`, `inbound_id`, `example_link`, `flow`) VALUES ('$code', '$text', 'none', 'offflow')");
                }
                sendMessage($from_id, "✅ با موفقیت تنظیم شد !", $manage_server);
            } else {
                sendMessage($from_id, "❌ اینباندی با ایدی <code>$text</code> پیدا نشد !", $back_panel);
    ...
    elseif (strpos($user['step'], 'set_example_link_alireza') !== false) {
        if (strpos($text, '%s1') !== false and strpos($text, '%s3') !== false) {
            step('none');
            $code = explode('-', $user['step'])[1];
            if ($sql->query("SELECT * FROM `alireza_panel_setting` WHERE `code` = '$code'")->num_rows > 0) {
                $sql->query("UPDATE `alireza_panel_setting` SET `example_link` = '$text' WHERE `code` = '$code'");
            } else {
                $sql->query("INSERT INTO `alireza_panel_setting` (`code`, `inbound_id`, `example_link`, `flow`) VALUES ('$code', 'none', '$text', 'offflow')");
            }
            sendMessage($from_id, "✅ با موفقیت تنظیم شد !", $manage_server);
        } else {
            sendMessage($from_id, "❌ نمونه لینک ارسالی شما اشتباه است !", $back_panel);
    ...
    elseif ($user['step'] == 'add_copen') {
      ...
      //  sendMessage($channel_id, "کد تخفیف جدید منتشر شد: <code>$text</code>" . $copen);
    }
    ...
    elseif ($user['step'] == 'send_percent') {
        if (is_numeric($text)) {
         ...
         //   sendMessage($channel_id, "درصد تخفیف: <code>$text</code>" . $copen);
        } else {
    ...
    elseif ($user['step'] == 'send_count_use') {
        if (is_numeric($text)) {
          ...
          //  sendMessage($channel_id, "تعداد مصرف : <code>$text</code>" . $copen);
